...
<script>
    $(document).ready(function() {
        ...
        var table = $('#dataTable').DataTable({
            ...
            columnDefs: [
                { orderable: false, targets: [0, 1, 5, 6] }, // Disable sorting for these columns
                { orderable: true, targets: [2, 3, 4] }, // Enable sorting for Type, Status, Created Date
                { 
                    type: 'status-sort', 
                    targets: 3 // Custom sorting for Status column
                }
            ],
            initComplete: function() {
                // Add sorting icons to sortable columns
                this.api().columns().header().each(function(header, index) {
                    if (table.settings()[0].aoColumns[index].bSortable) {
        ...
        // Custom sorting for status (Up before Down)
        $.fn.dataTable.ext.order['status-sort'] = function(settings, col) {
            return this.api().column(col, {order:'index'}).nodes().map(function(td, i) {
                return $(td).find('.badge').hasClass('badge-success') ? '0' : '1';
            });
        };
        ...
        function updateDashboardStats() {
            $.ajax({
                ...
                success: function(response) {
                    ...
                    // Optionally reload the table if you want fresh data
                    // table.ajax.reload(null, false);
                },
                error: function(xhr) {
                    console.error('Error updating dashboard stats:', xhr);
                }
            });
        }
